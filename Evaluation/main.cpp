...
int main(int argc, char* argv[]) {
	...
	glBufferData(GL_ARRAY_BUFFER, reference_data.size(), &(reference_data[0]), GL_STATIC_DRAW);

	glEnableVertexAttribArray(0);
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, reference_layout.vertex_size, (void*)reference_layout.x_offset);
	glVertexAttribPointer(1, 3, GL_UNSIGNED_BYTE, GL_TRUE, reference_layout.vertex_size, (void*)reference_layout.r_offset);
	...
	while (!should_exit) {
		...
		glDrawArrays(GL_POINTS, 0, reference_data.size() / reference_layout.vertex_size);
	}
